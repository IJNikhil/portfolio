...
import { TrophyIcon, CodeBracketIcon, ArrowRightIcon } from "@heroicons/react/24/outline";
...
export default function Showcase({ data, achievementsData }) {
  ...
  return (
    <section
      id="showcase"
      className="relative py-20 sm:py-24 bg-gradient-to-b from-[#0b0b0c] via-[#111113] to-[#0b0b0c] text-gray-100 overflow-hidden scroll-mt-16"
    >
      {/* Ambient glowing background */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/3 left-1/4 w-[300px] h-[300px] bg-[var(--accent)]/10 blur-[100px] rounded-full" />
        <div className="absolute bottom-1/3 right-1/4 w-[260px] h-[260px] bg-[var(--accent-secondary)]/10 blur-[90px] rounded-full" />
      </div>
      ...
      <Container className="relative z-10">
        ...
        <div className="text-center mb-2">
          ...
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-gray-400 text-base sm:text-lg max-w-2xl mx-auto mt-2"
          >
            A curated selection of my featured projects and milestone achievements.
          </motion.p>
        </div>

        {/* === Two-Column Layout with Gradient Divider === */}
        <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 mt-14">
          {/* Colorful divider */}
          <div
            className="hidden lg:block absolute left-1/2 top-0 h-full w-[2px] 
                       ...
                       opacity-70 rounded-full pointer-events-none shadow-[0_0_25px_var(--accent-secondary)/30]"
          ></div>

          {/* === Featured Projects === */}
          <div>
            <h3 className="text-lg sm:text-xl font-semibold mb-6 font-serif text-gray-200 tracking-wide flex items-center">
              <CodeBracketIcon className="h-5 w-5 text-[var(--accent)] mr-2" /> Featured Projects
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {displayProjects.map((project, i) => (
                ...
                >
                  ...
                  >
                    <GlassCard
                      className="relative overflow-hidden p-6 sm:p-7 border border-gray-700/40 rounded-2xl 
                                 bg-[#16161a]/70 backdrop-blur-md transition-all duration-300 
                                 hover:border-[var(--accent)]/50 hover:shadow-[0_0_25px_var(--accent)/20]"
                    >
                      {/* Accent gradient bar */}
                      <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-[var(--accent)] to-[var(--accent-secondary)] opacity-70" />
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="rounded-lg px-5 py-2 border-[var(--accent)] text-[var(--accent)] 
                                   hover:bg-[var(--accent)] hover:text-[var(--bg)]
                                   hover:shadow-[0_0_20px_var(--accent)/40] transition-all duration-300"
                      >
                        View Details <ArrowRightIcon className="w-4 h-4 ml-2 inline-block" />
                      </Button>
                    </GlassCard>
                  </Link>
                </motion.div>
                  ...
                  <AnimatedLink to="/projects/all">
                    <Button
                      ...
                      className="rounded-xl border border-[var(--accent)] text-[var(--accent)] 
                                 ...
                                 transition-all duration-300 hover:shadow-[0_0_25px_var(--accent)/40]"
                    >
                      View All Projects →
                    </Button>
                  </AnimatedLink>
          ...
          <div>
            <h3 className="text-lg sm:text-xl font-semibold mb-6 font-serif text-gray-200 tracking-wide flex items-center">
              <TrophyIcon className="h-5 w-5 text-yellow-400 mr-2" /> Milestones
            </h3>
            ...
            {displayAchievements.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {displayAchievements.map((achievement, i) => (
                  ...
                  >
                    <GlassCard
                      className="relative p-6 sm:p-7 border border-gray-700/40 rounded-2xl 
                                 bg-[#16161a]/70 backdrop-blur-md transition-all duration-300 
                                 hover:border-[var(--accent-secondary)]/50 hover:shadow-[0_0_25px_var(--accent-secondary)/20]"
                    >
                      {/* Accent gradient bar */}
                      <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-[var(--accent-secondary)] to-[var(--accent)] opacity-70" />

                      <h4
                        className="text-base sm:text-lg font-semibold font-serif text-white mb-2
                                   transition-colors duration-300 group-hover:text-[var(--accent-secondary)]"
                      >
                        {achievement.title}
                      </h4>
                      <p className="text-gray-400 text-sm leading-relaxed mb-4 line-clamp-3">
                      ...
                      {achievement.social && (
                        <Button
                          ...
                          className="rounded-lg px-5 py-2 border-[var(--accent-secondary)] text-[var(--accent-secondary)]
                                     hover:bg-[var(--accent-secondary)] hover:text-[var(--bg)]
                                     hover:shadow-[0_0_20px_var(--accent-secondary)/40] transition-all duration-300"
                        >
                          View Post →
                        </Button>
                      )}
